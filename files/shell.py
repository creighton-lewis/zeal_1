...
#Define target 
#Determine open ports on target machine: use nmap to determine
#Try to determine target machine type 
#Record in file 
#Start simple python server 
#Upload file specific to machine type
...
def ssh_connect():
    ...
    os.sysetm(f"ssh@{attack_val}")
    os.system(f"{pass_val}")
def target_define():
    target_ip = console.input("Enter target ip ")
    if '.com' in target_ip or '.org' in target_ip or '.net' in target_ip or '.edu' in target_ip:
        try:
            ...
            sys.exit(1)
    os.system(f"rustscan -a {target_ip} -- -o {target_ip}_port")
